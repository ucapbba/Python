...
class ModelAnalyser(object):
    ...
    def try_models_even(self,df : pd.DataFrame, model):
        ...
        for column in dataset_train.drop(['orbit'],axis=1).columns:
            dataset_train[column]= (dataset_train[column]- dataset_train[column].min())/(dataset_train[column].max()-dataset_train[column].min())
        data_orbit1 = dataset_train.loc[(dataset_train['orbit'] != 2)&(dataset_train['orbit'] != 3)&(dataset_train['orbit'] != 4)]
        data_orbit2 = dataset_train.loc[(dataset_train['orbit'] != 1)&(dataset_train['orbit'] != 3)&(dataset_train['orbit'] != 4)]
        data_orbit3 = dataset_train.loc[(dataset_train['orbit'] != 2)&(dataset_train['orbit'] != 1)&(dataset_train['orbit'] != 4)]
        data_orbit4 = dataset_train.loc[(dataset_train['orbit'] != 2)&(dataset_train['orbit'] != 3)&(dataset_train['orbit'] != 1)]
        dataset_train_even = pd.concat([data_orbit1.head(data_orbit4.shape[0]),data_orbit2.head(data_orbit4.shape[0]),data_orbit3.head(data_orbit4.shape[0]),data_orbit4])
    ...
    def stats_compare_models(self, df : pd.DataFrame, models : list, path):
        ...
        for i in models:
            ...
            stats_dict['precision'], stats_dict['accuracy'], stats_dict['sensitivity'], stats_dict['specificity'] = precision, accuracy, sensitivity, specificity
